...
- minimaal 1 `nl.novi.Person`
- minimaal 1 `nl.novi.Pet`
- alle methodes (behalve de getters en setters) worden getest door een unittest

De `nl.novi.Person` heeft volgende attributen:
...
- sex
- age
- mother (nl.novi.Person)
- father (nl.novi.Person)
- siblings (lijst)
- children (lijst)
- pets (lijst)

De `nl.novi.Pet` heeft minimaal de volgende attributen:

- name
- age
- species
- owner (nl.novi.Person)

De `nl.novi.Person` bevat naast de attributen het volgende:
...
De `nl.novi.Pet` bevat naast de attributen minimaal:
...
7. Schrijf unittesten voor alle getters en setters van zowel nl.novi.Pet als nl.novi.Person. 

8. Schrijf unittesten voor de extra methodes van nl.novi.Person (addPArents, addChild, addPet, addSibling, getGrandChildren)
...
3. Voeg een partner toe aan de `nl.novi.Person` en implementeer deze partner door de gehele applicatie inclusief getter & setter en benodigde methodes.
